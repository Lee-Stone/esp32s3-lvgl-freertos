...
void lv_100ask_nes_set_fn(lv_obj_t * obj, char * fn)
{
    ...
    if(nes->cur_fn) {
        if(strlen(nes->cur_fn) < fn_len) {
            heap_caps_free(nes->cur_fn);//lv_mem_free(nes->cur_fn);
            nes->cur_fn = NULL;
        }
        else {
    ...
    nes->cur_fn = heap_caps_malloc(fn_len + 1, MALLOC_CAP_SPIRAM);//lv_mem_alloc(fn_len + 1);
...
void lv_100ask_nes_flush(void)
{
    ...
    if(nes->state == LV_100ASK_NES_STATE_NORMAL) {
        ...
        vTaskDelay(lv_100ask_nes_get_speed(nes_obj)/1000);
    }
...
static void lv_100ask_nes_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    nes->dsc = heap_caps_malloc(sizeof(lv_img_dsc_t), MALLOC_CAP_SPIRAM);//lv_mem_alloc(sizeof(lv_img_dsc_t));
