...
#if LV_MEM_CUSTOM == 0
  // if(ROM == NULL)
  //   ROM = (BYTE *)heap_caps_malloc(NesHeader.byRomSize * 0x4000, MALLOC_CAP_SPIRAM);//( NesHeader.byRomSize * 0x4000 );
  // else
  //   ROM = (BYTE *)heap_caps_realloc(ROM, NesHeader.byRomSize * 0x4000, MALLOC_CAP_SPIRAM);//lv_mem_realloc(ROM, NesHeader.byRomSize * 0x4000 );
#else
  if(ROM == NULL)
...
#if LV_MEM_CUSTOM == 0
      // if(ROM == NULL)
      //   VROM = (BYTE *)heap_caps_malloc(NesHeader.byVRomSize * 0x2000 , MALLOC_CAP_SPIRAM);//lv_mem_alloc( NesHeader.byVRomSize * 0x2000 );
      // else
      //   VROM = (BYTE *)heap_caps_realloc(VROM, NesHeader.byVRomSize * 0x2000, MALLOC_CAP_SPIRAM);//lv_mem_realloc(VROM, NesHeader.byVRomSize * 0x2000 );
#else
      if(ROM == NULL)
...
void InfoNES_ReleaseRom(){
  if(ROM) {
#if LV_MEM_CUSTOM == 0
    // heap_caps_free(ROM);//lv_mem_free(ROM);
#else
    free(ROM);
#endif
  ...
  if(VROM){
#if LV_MEM_CUSTOM == 0
    // heap_caps_free(VROM);//lv_mem_free(VROM);
#else
    free(VROM);
#endif
