...
static void init_my_nes_front_end(lv_obj_t * parent)
{
    ...
    // lv_style_init(&nes_ctrl_btn_style)：初始化 nes_ctrl_btn_style 样式对象。
    // lv_style_set_bg_opa(&nes_ctrl_btn_style, LV_OPA_0)：设置背景的不透明度为 LV_OPA_0，即完全透明。
    // lv_style_set_pad_all(&nes_ctrl_btn_style, 4)：设置所有方向的内边距为 4 像素。
    // lv_style_set_radius(&nes_ctrl_btn_style, 0)：设置边框半径为 0，即直角。
    // lv_style_set_border_width(&nes_ctrl_btn_style, 0)：设置边框宽度为 0，即没有边框。

    lv_obj_t * ctrl_area = lv_obj_create(parent);
    lv_obj_remove_style_all(ctrl_area); //子对象继承父对象的样式
    lv_obj_set_size(ctrl_area, LV_PCT(100), LV_PCT(50));
    lv_obj_align(ctrl_area,LV_ALIGN_BOTTOM_MID, 0, 0);

    lv_obj_t * btnm_dir = lv_btnmatrix_create(ctrl_area); //矩阵按钮
    lv_obj_set_height(btnm_dir, LV_PCT(60)); //设置 btnm_dir 对象的高度为父对象的 60%
    lv_obj_align(btnm_dir, LV_ALIGN_LEFT_MID, 0, 0);
    lv_btnmatrix_set_map(btnm_dir, btnm_dir_map);
    lv_btnmatrix_set_btn_ctrl_all(btnm_dir, LV_BTNMATRIX_CTRL_CHECKED); //设置 btnm_dir 对象的所有按钮都具有都被选中
...
#else
    lv_100ask_file_explorer_open_dir(file_explorer, "S:/ROM/NES");
#endif
